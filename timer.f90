...
!
  ...
  implicit none
  private
  public TimerStart, TimerStop, TimerReport, IOStart, IOStop , ArrayStart, ArrayStop, ArrayMinus, MemoryReport, memory_limit
!
...
!
  ...
  !
    ...
    !
    subroutine TimerReport(real_time_tot)
      real(trk),intent(out) :: real_time_tot
      real(trk)          :: real_now
      real(trk)          :: cpu_now
      ...
      scan: do ord=1,t_count
        ...
        write (out,"(t2,a30,t33,a1,t35,f8.0,t45,2(f9.1,1x,f9.1,3x))") &
               ...
               !
        !
        real_time_tot = max(real_time_tot,real_time)
        !
      end do scan
      if (omitted>0) then
...
!
   ...
   !
    ...
    !  End an array-unit
    !
    subroutine ArrayMinus(comm,name)
      integer(ik), intent(in)      :: comm   ! communicator
      character(len=*), intent(in) :: name  ! Unit name
      !
      integer(ik)        :: pos        ! unit position
